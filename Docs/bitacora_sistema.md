...
- **[2026-07-02]** `[Ingeniería]`: FASE 4.5 — MIGRACIÓN AWS + BLINDAJE DE AFILIADOS + CASCADA COGNITIVA v4.4.
  - `[Contexto]`: Billing GCP irrecuperable (VM `instance-20260527-173717` inaccesible). Decisión: migrar hosting a AWS manteniendo Vertex AI/Gemini/Antigravity como cadena cognitiva.
  - `[providers]`: NUEVO `gemini_provider.py` — Gemini API por API key (AI Studio, free tier, SIN billing GCP), stdlib-only (urllib). `router.py` reescrito en cascada de 3 niveles: Vertex → Gemini API → Antigravity. `GEMINI_API_KEY`/`GEMINI_MODEL` cableados en docker-compose.yaml y docker-compose.local.yml.
  - `[middleware.ts]`: Geo-resolución multi-fuente (?geo → cookie cg_geo 30d → headers Vercel/CF → lookup ip-api.com → US). Allowlist estricta de dominios en `/out` (anti open-redirect). Todos los CTA de afiliado con `rel="sponsored noopener noreferrer"`.
  - `[Legal]`: Páginas Privacidad/Términos/Cookies/Afiliados en `/{locale}/legal/{slug}` (es/en/pt) desde `src/lib/legalContent.ts`. Divulgación de afiliados visible en footer (FTC/RGPD). Footer sin jerga técnica (API/GraphQL, Rust Axum, Zero-Trust eliminados de la UI pública).
  - `[UI]`: Noticias movidas arriba (bajo el Hero, estilo NVIDIA) con carrusel auto-avance 3.5s (pausa hover, reduced-motion). Semillas de noticias geolocalizadas (`Infra/db/seeds_news.sql`: 5 globales + AR/ES/US/MX/BR). Tipografía: Space Grotesk en h1–h3 + contraste de grises elevado un paso.
  - `[Infra]`: NUEVO `Infra/aws/` — README de migración + `deploy-ec2.sh` (EC2 Ubuntu 22.04 + Docker Compose, stack completo). Fix compose local: seeds.sql → seeds_catalog.sql, `postgres_db` → `postgres-db` (Ruby URI rechaza `_` en hostnames), puerto 3000 expuesto en localhost.
  - `[next.config.ts]`: Headers de caché agresiva e imágenes optimizadas solo en producción (en dev rompían hot-reload y el proxy de imágenes). Dominios de imagen completados (MSI/Asus/Acer/Apple/Razer/www8-hp).

- **[2026-07-02]** `[Ingeniería]`: GEMINI API ACTIVADO + APP MÓVIL (FASE 5 INICIADA).
  - `[Gemini]`: `GEMINI_API_KEY` configurada en `.env` y probada end-to-end: la key del free tier funciona a pesar del billing GCP suspendido. `GeminiProvider` verificado con salida JSON estructurada real (`gemini-2.5-flash`). Los agentes ya operan con inteligencia real, no solo Antigravity. ⚠️ Pendiente: rotar la key (fue expuesta en captura) y idealmente re-crearla en un proyecto AI Studio nuevo, no en `clicks-and-go`.
  - `[Rails]`: NUEVO `GET /api/v1/geo` (`geo_controller.rb`) — resolución de país compartida Web + Mobile: override `?country` → headers de plataforma (incluye `cloudfront-viewer-country` para AWS) → lookup ip-api cacheado 24h por IP → US. Devuelve `{country_code, currency, locale, supported_countries}`.
  - `[Mobile]` NUEVO: App React Native + Expo (TypeScript) en `Mobile/`. Pantallas: catálogo geolocalizado (precio local + ref USD + deal score) y noticias geolocalizadas, selector de región con banderas, pull-to-refresh, i18n es/en/pt automático según geo, tema oscuro espejo de la web. **Afiliación blindada**: el botón de compra abre `WEB_BASE_URL/out?url=...` (mismo gateway con allowlist + tags que la web — la monetización nunca se saltea). `npx tsc --noEmit` ✅. Config de entorno en `Mobile/src/config.ts` (API_BASE_URL / WEB_BASE_URL).
  - `[Verificación]`: Flujo completo simulado: geo(MX) → 7 laptops MXN + 7 noticias (regionales+globales). Identificadores: `com.clicksandgo.app` (Android/iOS).
