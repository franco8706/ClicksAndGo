...
* **🖼️ Política de imágenes reales (sin mock data):** el sitio no muestra NINGUNA imagen que no sea la foto real del producto — ni de bancos de stock, ni decorativa, ni de otro artículo (representación engañosa: FTC §5, Directiva 2005/29/CE, Ley 24.240 art. 4). Sin foto real, `urls.image` es `null` y la UI dibuja el ícono neutro de la categoría (`ProductImage.tsx`). Se garantiza en **4 capas con la misma lista de hosts**: (1) ingesta — `clean_image_url` en `data_normalizer.py` verifica por HTTP que la URL devuelva `200` + `Content-Type: image/*` antes de persistir; (2) Postgres — CHECK `chk_laptops_no_stock_image`; (3) Rails — `Laptop#real_image_url`; (4) Web — `src/lib/productImage.ts` + allowlist de `next.config.ts` (sin bancos de stock, así el optimizador rechaza con 400). Los seeds **no siembran imágenes**: entran solo por el pipeline (feeds de afiliados / Amazon PA-API).
